...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_UPD_CREATURE_TEMPLATE_NAME, "UPDATE creature_template SET name = ? WHERE entry = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(WORLD_DEL_CREATURE_PLAYERBYTES, "DELETE FROM creature_playerbytes WHERE entry = ?", CONNECTION_ASYNC);
    PrepareStatement(WORLD_INS_CREATURE_PLAYERBYTES,
        "INSERT INTO creature_playerbytes (entry, race, class, gender, playerBytes, playerBytes2, guildId, "
    ...
    PrepareStatement(WORLD_SEL_CREATURE_TEMPLATE, "SELECT ct.entry, ct.difficulty_entry_1, ct.difficulty_entry_2, ct.difficulty_entry_3, ct.KillCredit1, ct.KillCredit2, ct.modelid1, ct.modelid2, ct.modelid3, ct.modelid4, ct.name, ct.subname, ct.IconName, ct.gossip_menu_id, ct.minlevel, ct.maxlevel, ct.exp, ct.faction, ct.npcflag, ct.speed_walk, ct.speed_run, ct.scale, ct.`rank`, ct.dmgschool, ct.BaseAttackTime, ct.RangeAttackTime, ct.BaseVariance, ct.RangeVariance, ct.unit_class, ct.unit_flags, ct.unit_flags2, ct.dynamicflags, ct.family, ct.type, ct.type_flags, ct.lootid, ct.pickpocketloot, ct.skinloot, ct.PetSpellDataId, ct.VehicleId, ct.mingold, ct.maxgold, ct.AIName, ct.MovementType, ctm.Ground, ctm.Swim, ctm.Flight, ctm.Rooted, ctm.Chase, ctm.Random, ctm.InteractionPauseTimer, ct.HoverHeight, ct.HealthModifier, ct.ManaModifier, ct.ArmorModifier, ct.DamageModifier, ct.ExperienceModifier, ct.RacialLeader, ct.movementId, ct.RegenHealth, IFNULL(ctl.DisableLootTag, 0) AS DisableLootTag, ct.mechanic_immune_mask, ct.spell_school_immune_mask, ct.flags_extra, ct.ScriptName FROM creature_template ct LEFT JOIN creature_template_movement ctm ON ct.entry = ctm.CreatureId LEFT JOIN creature_template_loot_flags ctl ON ct.entry = ctl.Entry WHERE ct.entry = ?", CONNECTION_SYNCH);
