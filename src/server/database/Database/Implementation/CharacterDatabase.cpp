...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    // Same row, without the "is the new name free" test: a family name makes
    // the answer a pair, which the character cache knows and this cannot ask
    // for without naming a column some realms do not have (Surnames.h).
    PrepareStatement(CHAR_SEL_RENAME_TARGET, "SELECT guid, name, at_login FROM characters WHERE guid = ? AND account = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_APPEARANCE_BY_GUID, "SELECT name, race, class, gender, skin, face, hairStyle, hairColor, facialStyle, playerFlags, equipmentCache FROM characters WHERE guid = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_HAS_SPELL, "SELECT 1 FROM character_spell WHERE guid = ? AND spell = ? LIMIT 1", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_QUESTSTATUS_DAILY, "SELECT quest, time FROM character_queststatus_daily cq join characters c on c.account = cq.guid WHERE c.guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_CHARACTER_QUESTSTATUS_DAILY, "DELETE FROM character_queststatus_daily where guid = (select account from characters c where c.guid = ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHARACTER_QUESTSTATUS_DAILY, "INSERT INTO character_queststatus_daily (guid, quest, time) select account, ?, ? from characters where guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHAR_KNOWN_TITLES, "SELECT knownTitles, chosenTitle FROM characters WHERE guid = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_UPD_CHAR_KNOWN_TITLES, "UPDATE characters SET knownTitles = ?, chosenTitle = ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_WEEKLY_HONOR_TOP_THREE, "SELECT chw.guid, chw.weekly_honor, c.account FROM character_honor_weekly chw INNER JOIN characters c ON chw.guid = c.guid WHERE chw.weekly_honor > 0 ORDER BY chw.weekly_honor DESC, chw.guid ASC LIMIT 50", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_CHAR_SKILL_COUNT_BY_GUID, "SELECT COUNT(*) FROM character_skills WHERE guid = ?", CONNECTION_SYNCH);
    ...
    // Arena replay storage is implemented in scripts/Custom/BGReplay.cpp using character_arena_replays.
}
