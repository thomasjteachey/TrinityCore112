...
BattlegroundSV::BattlegroundSV()
{
	...
	StartMessageIds[BG_STARTING_EVENT_FIRST] = BG_SV_BROADCAST_START_TWO_MINUTES;
	StartMessageIds[BG_STARTING_EVENT_SECOND] = BG_SV_BROADCAST_START_ONE_MINUTE;
	StartMessageIds[BG_STARTING_EVENT_THIRD] = BG_SV_BROADCAST_START_HALF_MINUTE;
	StartMessageIds[BG_STARTING_EVENT_FOURTH] = BG_SV_BROADCAST_HAS_BEGUN;
...
bool BattlegroundSV::SetupBattleground()
{
	...
	for (uint8 i = 0; i < MAX_OBJECT_BANNER_SPAWNS; ++i)
	{
		...
		{
			TC_LOG_INFO("module", "Slavery Valley: There was an error spawning gameobject {}", BG_SV_ObjSpawnlocs[i].entry);
			return false;
		}
	}
...
void BattlegroundSV::EventPlayerClickedOnFlag(Player *player, GameObject *target_obj)
{
	...
	for (uint8 i = 0; i < BG_SV_MAX_NODE_TYPES; ++i)
	{
		if (nodePoint[i].gameobject_entry == target_obj->GetEntry())
		{
			...
			{
				TC_LOG_INFO("module", "Slavery Valley: There was an error spawning a banner (type: {}, entry: {}). Slavery Valley BG cancelled.", nodePoint[i].gameobject_type, nodePoint[i].gameobject_entry);
...
uint32 BattlegroundSV::GetNextBanner(BG_SV_NodePoint *node, uint32 team, bool returnDefinitve)
{
	...
	TC_LOG_INFO("module", "Slavery Valley: Unexpected return in GetNextBanner function");
	return 0;
}
...
WorldSafeLocsEntry const* BattlegroundSV::GetClosestGraveyard(Player* player)
{
    ...
    for (uint8 i = 0; i < BG_SV_MAX_NODE_TYPES; ++i)
        if (nodePoint[i].captured && nodePoint[i].faction == teamIndex)
            nodes.push_back(i);

    WorldSafeLocsEntry const* entry = sWorldSafeLocsStore.LookupEntry(BG_SV_GraveyardIds[teamIndex + BG_SV_MAX_NODE_TYPES]);
    ...
    if (!nodes.empty())
    {
        ...
        {
            entry = sWorldSafeLocsStore.LookupEntry(BG_SV_GraveyardIds[nodes[i]]);
            if (!entry)
                continue;
