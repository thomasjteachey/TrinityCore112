...
void BattlegroundWS::EventPlayerCapturedFlag(Player* player)
{
    ...
    UpdateTeamScore(Battleground::GetTeamIndexByTeamId(player->GetBGTeam()));
...
{
    uint32 const worldStateId = (team == ALLIANCE) ? BG_WS_FLAG_STATE_ALLIANCE : BG_WS_FLAG_STATE_HORDE;

    // Prevent revealing the enemy flag carrier on the minimap by never sending the
    // "on player" state to clients. The client displays the minimap icon when this
    // world state is set to BG_WS_FLAG_STATE_ON_PLAYER, so fall back to
    // BG_WS_FLAG_STATE_WAIT_RESPAWN instead.
    uint32 const broadcastValue = (value == BG_WS_FLAG_STATE_ON_PLAYER) ? BG_WS_FLAG_STATE_WAIT_RESPAWN : value;

    UpdateWorldState(worldStateId, broadcastValue);
}
...
void BattlegroundWS::FillInitialWorldStates(WorldPackets::WorldState::InitWorldStates& packet)
{
    ...
    if (_flagState[TEAM_HORDE] == BG_WS_FLAG_STATE_ON_PLAYER)
        packet.Worldstates.emplace_back(BG_WS_FLAG_STATE_HORDE, BG_WS_FLAG_STATE_WAIT_RESPAWN);
    else
        packet.Worldstates.emplace_back(BG_WS_FLAG_STATE_HORDE, 1);

    if (_flagState[TEAM_ALLIANCE] == BG_WS_FLAG_STATE_ON_PLAYER)
        packet.Worldstates.emplace_back(BG_WS_FLAG_STATE_ALLIANCE, BG_WS_FLAG_STATE_WAIT_RESPAWN);
    else
