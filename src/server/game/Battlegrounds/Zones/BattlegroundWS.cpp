...
void BattlegroundWS::EventPlayerCapturedFlag(Player* player)
{
    ...
    RewardHonorToTeam(flag_honor, team);
...
void BattlegroundWS::HandleAreaTrigger(Player* player, uint32 trigger)
{
    ...
    switch (trigger)
    {
        ...
        case 3646:                                          // Alliance Flag spawn
            if (team == ALLIANCE && _flagState[TEAM_HORDE] && !_flagState[TEAM_ALLIANCE])
                if (GetFlagPickerGUID(TEAM_HORDE) == player->GetGUID())
                    EventPlayerCapturedFlag(player);
            break;
        case 3647:                                          // Horde Flag spawn
            if (team == HORDE && _flagState[TEAM_ALLIANCE] && !_flagState[TEAM_HORDE])
                if (GetFlagPickerGUID(TEAM_ALLIANCE) == player->GetGUID())
                    EventPlayerCapturedFlag(player);
            break;
        case 3649:                                          // unk1
...
bool BattlegroundWS::SetupBattleground()
{
    ...
    if (!AddObject(BG_WS_OBJECT_A_FLAG, BG_OBJECT_A_FLAG_WS_ENTRY, 1540.423f, 1481.325f, 351.8284f, 3.089233f, 0, 0, 0.9996573f, 0.02617699f, BG_WS_FLAG_RESPAWN_TIME/1000)
        ...
        // buffs
        || !AddObject(BG_WS_OBJECT_SPEEDBUFF_1, BG_OBJECTID_SPEEDBUFF_ENTRY, 1449.93f, 1470.71f, 342.6346f, -1.64061f, 0, 0, 0.7313537f, -0.6819983f, BG_WS_SPEED_BUFF_RESPAWN_TIME)
        || !AddObject(BG_WS_OBJECT_SPEEDBUFF_2, BG_OBJECTID_SPEEDBUFF_ENTRY, 1005.171f, 1447.946f, 335.9032f, 1.64061f, 0, 0, 0.7313537f, 0.6819984f, BG_WS_SPEED_BUFF_RESPAWN_TIME)
        || !AddObject(BG_WS_OBJECT_REGENBUFF_1, BG_OBJECTID_REGENBUFF_ENTRY, 1317.506f, 1550.851f, 313.2344f, -0.2617996f, 0, 0, 0.1305263f, -0.9914448f, BG_WS_RESTORATION_BUFF_RESPAWN_TIME)
        || !AddObject(BG_WS_OBJECT_REGENBUFF_2, BG_OBJECTID_REGENBUFF_ENTRY, 1110.451f, 1353.656f, 316.5181f, -0.6806787f, 0, 0, 0.333807f, -0.9426414f, BG_WS_RESTORATION_BUFF_RESPAWN_TIME)
        || !AddObject(BG_WS_OBJECT_BERSERKBUFF_1, BG_OBJECTID_BERSERKERBUFF_ENTRY, 1320.09f, 1378.79f, 314.7532f, 1.186824f, 0, 0, 0.5591929f, 0.8290376f, BG_WS_BERSERKING_BUFF_RESPAWN_TIME)
        || !AddObject(BG_WS_OBJECT_BERSERKBUFF_2, BG_OBJECTID_BERSERKERBUFF_ENTRY, 1139.688f, 1560.288f, 306.8432f, -2.443461f, 0, 0, 0.9396926f, -0.3420201f, BG_WS_BERSERKING_BUFF_RESPAWN_TIME)
        // alliance gates
...
uint32 BattlegroundWS::GetBuffRespawnTime(uint32 type) const
{
    switch (type)
    {
        case BG_WS_OBJECT_REGENBUFF_1:
        case BG_WS_OBJECT_REGENBUFF_2:
            return BG_WS_RESTORATION_BUFF_RESPAWN_TIME;
        case BG_WS_OBJECT_BERSERKBUFF_1:
        case BG_WS_OBJECT_BERSERKBUFF_2:
            return BG_WS_BERSERKING_BUFF_RESPAWN_TIME;
        case BG_WS_OBJECT_SPEEDBUFF_1:
        case BG_WS_OBJECT_SPEEDBUFF_2:
            return BG_WS_SPEED_BUFF_RESPAWN_TIME;
        default:
            return Battleground::GetBuffRespawnTime(type);
    }
}
