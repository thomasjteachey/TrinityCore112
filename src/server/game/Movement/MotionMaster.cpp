...
void MotionMaster::MoveJump(Position const& pos, float speedXY, float speedZ, uint32 id/* = EVENT_JUMP*/, bool hasOrientation/* = false*/, bool orientationFixed/* = false*/, bool allowTurning/* = false*/)
{
    MoveJump(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation(), speedXY, speedZ, id, hasOrientation, orientationFixed, allowTurning);
}

void MotionMaster::MoveJump(float x, float y, float z, float o, float speedXY, float speedZ, uint32 id, bool hasOrientation /* = false*/, bool orientationFixed /* = false*/, bool allowTurning /* = false*/)
{
    ...
    movement->Priority = MOTION_PRIORITY_HIGHEST;
    movement->BaseUnitState = allowTurning ? UNIT_STATE_MOVE : UNIT_STATE_JUMPING;
    movement->AddFlag(MOVEMENTGENERATOR_FLAG_PERSIST_ON_DEATH);
    Add(movement);
}
