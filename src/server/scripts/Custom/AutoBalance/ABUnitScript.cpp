...
void AutoBalance_UnitScript::OnAuraApply(Unit* unit, Aura* aura)
{
    ...
    if (aura && (aura->GetDuration() > 0 || aura->GetMaxDuration() > 0))
    {
        ...
        {
            if (_debug_damage_and_healing)
                TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::OnAuraApply(): Spell '{}' had it's duration adjusted ({}->{}).",
...
void AutoBalance_UnitScript::_Debug_Output(std::string function_name, Unit* target, Unit* source, int32 amount, Damage_Healing_Debug_Phase phase, std::string spell_name, uint32 spell_id)
{
    ...
    {
        TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance:: ------------------------------------------------");
    }

    if (target && source && amount)
    {
        TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::{}: {}: {}{} {} {}{} with {}{} for ({})",
            function_name,
            phase ? "AFTER" : "BEFORE",
            source->GetName(),
    ...
    {
        TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::{}: {}: {}{} damages {}{} with {}{} for (0)",
            function_name,
            phase ? "AFTER" : "BEFORE",
            source->GetName(),
            source->GetEntry() ? " (" + std::to_string(source->GetEntry()) + ")" : "",
    ...
    {
        TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::{}: {}: ?? affects {}{} with {}{}",
            function_name,
            phase ? "AFTER" : "BEFORE",
            target->GetName(),
            target->GetEntry() ? " (" + std::to_string(target->GetEntry()) + ")" : "",
    ...
    {
        TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::{}: {}: W? T? F? with {}{}",
            function_name,
            phase ? "AFTER" : "BEFORE",
            spell_name,
            spell_id ? " (" + std::to_string(spell_id) + ")" : ""
...
int32 AutoBalance_UnitScript::_Modify_Damage_Healing(Unit* target, Unit* source, int32 amount, SpellInfo const* spellInfo)
{
    ...
    {
        if (_debug_damage_and_healing)
            TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_Modify_Damage_Healing: EnableGlobal is false, returning original value of ({}).", amount);
    ...
    {
        if (_debug_damage_and_healing)
            TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_Modify_Damage_Healing: Source or Target's map is not enabled, returning original value of ({}).", amount);
    ...
    {
        if (_debug_damage_and_healing)
            TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_Modify_Damage_Healing: Source is a player that is self-healing, returning original value of ({}).", amount);
    ...
    else if (source->GetTypeId() == TYPEID_PLAYER && source->GetGUID() == target->GetGUID() && amount < 0)
    {
        ...
        {
            if (_debug_damage_and_healing)
                TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_Modify_Damage_Healing: Source is a player that is self-damaging with a spell that is ignored, returning original value of ({}).", amount);

            return amount;
        }

        if (_debug_damage_and_healing)
            TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_Modify_Damage_Healing: Source is a player that is self-damaging, continuing.");
    }
    ...
    {
        if (_debug_damage_and_healing)
            TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_Modify_Damage_Healing: Source is a player that is damaging a friendly unit, continuing.");
    }
    // if the source is a player under any other condition, return the original value
    else if (source->GetTypeId() == TYPEID_PLAYER)
    {
        if (_debug_damage_and_healing)
            TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_Modify_Damage_Healing: Source is an enemy player, returning original value of ({}).", amount);
    ...
    {
        if (_debug_damage_and_healing)
            TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_Modify_Damage_Healing: Source is a creature that is self-damaging with an aura that shares damage, returning original value of ({}).", amount);
    ...
    {
        if (_debug_damage_and_healing)
            TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_Modify_Damage_Healing: Source is a player-controlled pet or summon, returning original value of ({}).", amount);
    ...
    if (source->GetTypeId() == TYPEID_PLAYER && source->GetGUID() == target->GetGUID() && amount < 0)
    {
        ...
        if (_isAuraWithEffectType(spellInfo, SPELL_AURA_PERIODIC_DAMAGE_PERCENT))
        {
            ...
            if (_debug_damage_and_healing)
            {
                TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_Modify_Damage_Healing: Spell damage based on percent of max health. Ignore level scaling.");
                TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC",
...
bool AutoBalance_UnitScript::_isAuraWithEffectType(SpellInfo const* spellInfo, AuraType auraType, bool log)
{
    ...
    {
        if (log)
            TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_isAuraWithEffectType: SpellInfo is null, returning false.");
        return false;
    }
    ...
    {
        if (log)
            TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_isAuraWithEffectType: SpellInfo has no effects, returning false.");
        return false;
    }
    ...
    for (SpellEffectInfo effect : spellInfo->GetEffects())
    {
        ...
        {
            if (log)
                TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_isAuraWithEffectType: SpellInfo has an effect that is not an aura, continuing to next effect.");
            continue;
        }
        ...
        if (effect.ApplyAuraName == auraType)
        {
            ...
            TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_isAuraWithEffectType: SpellInfo has an aura of the target type, returning true.");
            return true;
        }
    }
    ...
    if (log)
        TC_LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_isAuraWithEffectType: SpellInfo has no aura of the target type, returning false.");
    return false;
}
