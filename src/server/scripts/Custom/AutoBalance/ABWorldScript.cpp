...
void AutoBalance_WorldScript::OnConfigLoad(bool /*reload*/)
{
    std::string error;
    std::string configDir;
    std::string const& worldConfig = sConfigMgr->GetFilename();
    if (!worldConfig.empty())
    {
        size_t const pos = worldConfig.find_last_of("/\\");
        if (pos != std::string::npos)
            configDir = worldConfig.substr(0, pos + 1);
    }

    if (configDir.empty())
        configDir = "";

    std::string const autoBalanceConfig = configDir + "AutoBalance.conf";
    if (!sConfigMgr->LoadAdditionalFile(autoBalanceConfig, false, error))
        TC_LOG_WARN("server.loading", "mod-autobalance: unable to load configuration file '{}': {}", autoBalanceConfig, error);

    SetInitialWorldSettings();
    globalConfigTime = GetCurrentConfigTime();

    TC_LOG_INFO("module.AutoBalance", "AutoBalance::OnConfigLoad: Config loaded. Global config time set to ({}).", globalConfigTime);
}
...
void AutoBalance_WorldScript::SetInitialWorldSettings()
{
    ...
    if (sConfigMgr->GetOption<float>("AutoBalance.PerDungeonPlayerCounts", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.PerDungeonPlayerCounts` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    ...
    if (sConfigMgr->GetOption<float>("AutoBalance.PerDungeonScaling", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.PerDungeonScaling` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    ...
    if (sConfigMgr->GetOption<float>("AutoBalance.PerDungeonBossScaling", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.PerDungeonBossScaling` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    ...
    if (sConfigMgr->GetOption<int>("AutoBalance.enable", -1, false) != -1)
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.enable` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    ...
    if (sConfigMgr->GetOption<int>("AutoBalance.DungeonsOnly", -1, false) != -1)
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.DungeonsOnly` defined in `AutoBalance.conf`. This variable has been removed and has no effect. Please see `AutoBalance.conf.dist` for more details.");

    if (sConfigMgr->GetOption<int>("AutoBalance.levelUseDbValuesWhenExists", -1, false) != -1)
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.levelUseDbValuesWhenExists` defined in `AutoBalance.conf`. This variable has been removed and has no effect. Please see `AutoBalance.conf.dist` for more details.");
    ...
    if (sConfigMgr->GetOption<float>("AutoBalance.BossInflectionMult", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.BossInflectionMult` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    ...
    if (sConfigMgr->GetOption<float>("AutoBalance.rate.global", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.rate.global` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    if (sConfigMgr->GetOption<float>("AutoBalance.rate.health", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.rate.health` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    if (sConfigMgr->GetOption<float>("AutoBalance.rate.mana", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.rate.mana` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    if (sConfigMgr->GetOption<float>("AutoBalance.rate.armor", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.rate.armor` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    if (sConfigMgr->GetOption<float>("AutoBalance.rate.damage", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.rate.damage` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    ...
    {
        TC_LOG_ERROR("server.loading", "mod-autobalance: invalid value `{}` for `AutoBalance.LevelScaling.Method` defined in `AutoBalance.conf`. Defaulting to a value of `dynamic`.", LevelScalingMethodString);
        LevelScalingMethod = AUTOBALANCE_SCALING_DYNAMIC;
    }

    if (sConfigMgr->GetOption<float>("AutoBalance.LevelHigherOffset", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.LevelHigherOffset` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    ...
    if (sConfigMgr->GetOption<float>("AutoBalance.LevelLowerOffset", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.LevelLowerOffset` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    ...
    if (sConfigMgr->GetOption<float>("AutoBalance.LevelEndGameBoost", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.LevelEndGameBoost` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    ...
    {
        TC_LOG_WARN("server.loading", "mod-autobalance: `AutoBalance.LevelScaling.EndGameBoost` is enabled in the configuration, but is not currently implemented. No effect.");
    ...
    if (sConfigMgr->GetOption<float>("AutoBalance.DungeonScaleDownXP", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.DungeonScaleDownXP` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");

    if (sConfigMgr->GetOption<float>("AutoBalance.DungeonScaleDownMoney", false, false))
        TC_LOG_WARN("server.loading", "mod-autobalance: deprecated value `AutoBalance.DungeonScaleDownMoney` defined in `AutoBalance.conf`. This variable will be removed in a future release. Please see `AutoBalance.conf.dist` for more details.");
    ...
    {
        TC_LOG_ERROR("server.loading", "mod-autobalance: invalid value `{}` for `AutoBalance.RewardScaling.Method` defined in `AutoBalance.conf`. Defaulting to a value of `dynamic`.", RewardScalingMethodString);
        RewardScalingMethod = AUTOBALANCE_SCALING_DYNAMIC;
    }
