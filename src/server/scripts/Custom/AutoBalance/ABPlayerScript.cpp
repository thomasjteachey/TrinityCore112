...
void AutoBalance_PlayerScript::OnPlayerLevelChanged(Player* player, uint8 oldlevel)
{
    ...
    TC_LOG_DEBUG("module.AutoBalance", "AutoBalance_PlayerScript::OnLevelChanged: {} has leveled ({}->{})", player->GetName(), oldlevel, player->GetLevel());
...
void AutoBalance_PlayerScript::OnPlayerGiveXP(Player* player, uint32& amount, Unit* victim, uint8 /*xpSource*/)
{
    ...
    if (victim && RewardScalingXP && mapABInfo->enabled)
    {
        ...
        if (map->IsDungeon())
        {
            ...
            {
                TC_LOG_DEBUG("module.AutoBalance", "AutoBalance_PlayerScript::OnGiveXP: Distributing XP from '{}' to '{}' in dynamic mode - {}->{}",
            ...
            else if (RewardScalingMethod == AUTOBALANCE_SCALING_FIXED)
            {
                ...
                TC_LOG_DEBUG("module.AutoBalance", "AutoBalance_PlayerScript::OnGiveXP: Distributing XP from '{}' to '{}' in fixed mode - {}->{}",
...
void AutoBalance_PlayerScript::OnPlayerBeforeLootMoney(Player* player, Loot* loot)
{
    ...
    if (mapABInfo->enabled && RewardScalingMoney)
    {
        ...
        if (sourceGuid.IsCreature())
        {
            ...
            if (RewardScalingMethod == AUTOBALANCE_SCALING_DYNAMIC)
            {
                TC_LOG_DEBUG("module.AutoBalance", "AutoBalance_PlayerScript::OnBeforeLootMoney: Distributing money from '{}' in dynamic mode - {}->{}",
            ...
            else if (RewardScalingMethod == AUTOBALANCE_SCALING_FIXED)
            {
                ...
                auto maxPlayerCount = map->ToInstanceMap()->GetMaxPlayers();
                auto currentPlayerCount = mapABInfo->playerCount;
                TC_LOG_DEBUG("module.AutoBalance", "AutoBalance_PlayerScript::OnBeforeLootMoney: Distributing money from '{}' in fixed mode - {}->{}",
        ...
        else
        {
            auto maxPlayerCount = map->ToInstanceMap()->GetMaxPlayers();
            auto currentPlayerCount = mapABInfo->playerCount;
            TC_LOG_DEBUG("module.AutoBalance", "AutoBalance_PlayerScript::OnBeforeLootMoney: Distributing money from a non-creature in fixed mode - {}->{}",
...
void AutoBalance_PlayerScript::OnPlayerEnterCombat(Player* player, Unit* /*enemy*/)
{
    ...
    TC_LOG_DEBUG("module.AutoBalance_CombatLocking", "AutoBalance_PlayerScript::OnPlayerEnterCombat: {} enters combat.", player->GetName());
    ...
    if (!mapABInfo->combatLocked)
    {
        ...
        TC_LOG_DEBUG("module.AutoBalance_CombatLocking", "AutoBalance_PlayerScript::OnPlayerEnterCombat: Map {} ({}{}) | Locking difficulty to no less than ({}) as {} enters combat.",
            map->GetMapName(),
            map->GetId(),
...
void AutoBalance_PlayerScript::OnPlayerLeaveCombat(Player* player)
{
    ...
    TC_LOG_DEBUG("module.AutoBalance_CombatLocking", "AutoBalance_PlayerScript::OnPlayerLeaveCombat: {} leaves (or wasn't in) combat.", player->GetName());
    ...
    for (auto player : mapABInfo->allMapPlayers)
    {
        if (player && player->IsInCombat())
        {
            ...
            TC_LOG_DEBUG("module.AutoBalance_CombatLocking", "AutoBalance_PlayerScript::OnPlayerLeaveCombat: Map {} ({}{}) | Player {} (and potentially others) are still in combat.",
                map->GetMapName(),
                map->GetId(),
                map->GetInstanceId() ? "-" + std::to_string(map->GetInstanceId()) : "",
                player->GetName()
    ...
    if (!anyPlayersInCombat && mapABInfo->combatLocked)
    {
        ...
        TC_LOG_DEBUG("module.AutoBalance_CombatLocking", "AutoBalance_PlayerScript::OnPlayerLeaveCombat: Map {} ({}{}) | Unlocking difficulty as {} leaves combat.",
            map->GetMapName(),
            map->GetId(),
            map->GetInstanceId() ? "-" + std::to_string(map->GetInstanceId()) : "",
            player->GetName()
        ...
        if (mapABInfo->combatLockTripped && mapABInfo->playerCount != mapABInfo->combatLockMinPlayers)
        {
            ...
            TC_LOG_DEBUG("module.AutoBalance_CombatLocking", "AutoBalance_PlayerScript::OnPlayerLeaveCombat: Map {} ({}{}) | Reset map config time to ({}).",
                map->GetMapName(),
                map->GetId(),
                map->GetInstanceId() ? "-" + std::to_string(map->GetInstanceId()) : "",
