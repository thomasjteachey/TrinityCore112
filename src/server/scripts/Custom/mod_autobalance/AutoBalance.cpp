...
#ifndef LOG_DEBUG
#define LOG_DEBUG(filter, ...) TC_LOG_DEBUG(filter, __VA_ARGS__)
#endif
#ifndef LOG_INFO
#define LOG_INFO(filter, ...) TC_LOG_INFO(filter, __VA_ARGS__)
#endif
#ifndef LOG_WARN
#define LOG_WARN(filter, ...) TC_LOG_WARN(filter, __VA_ARGS__)
#endif
#ifndef LOG_ERROR
#define LOG_ERROR(filter, ...) TC_LOG_ERROR(filter, __VA_ARGS__)
#endif
...
class AutoBalanceStatModifiers
{
public:
...
class AutoBalanceInflectionPointSettings
{
public:
...
class AutoBalanceLevelScalingDynamicLevelSettings
{
public:
...
namespace
{
    std::unordered_map<Creature*, AutoBalanceCreatureInfo> sAutoBalanceCreatureData;
    std::unordered_map<Map*, AutoBalanceMapInfo> sAutoBalanceMapData;

    AutoBalanceCreatureInfo* GetAutoBalanceCreatureInfo(Creature* creature)
    {
        return &sAutoBalanceCreatureData[creature];
    }

    AutoBalanceMapInfo* GetAutoBalanceMapInfo(Map* map)
    {
        return &sAutoBalanceMapData[map];
    }

    void ResetAutoBalanceCreatureInfo(Creature* creature)
    {
        sAutoBalanceCreatureData.erase(creature);
    }

    void ResetAutoBalanceMapInfo(Map* map)
    {
        sAutoBalanceMapData.erase(map);
    }
}
...
bool isCreatureRelevant(Creature* creature) {
    ...
    AutoBalanceCreatureInfo *creatureABInfo=GetAutoBalanceCreatureInfo(creature);
    ...
    AutoBalanceMapInfo *mapABInfo=GetAutoBalanceMapInfo(creatureMap);
        ...
        for (std::vector<Player*>::const_iterator playerIterator = mapABInfo->allMapPlayers.begin(); playerIterator != mapABInfo->allMapPlayers.end(); ++playerIterator)
        {
            ...
            )
            {
                ...
                // unfortunately, `creature->IsHostileTo(thisPlayer)` returns true for cases when it is not actually hostile
                    ...
                    thisPlayer->isTargetableForAttack(true)
                )
                {
...
AutoBalanceInflectionPointSettings getInflectionPointSettings (InstanceMap* instanceMap, bool isBoss = false)
{
    ...
    uint32 mapId = instanceMap->GetEntry()->ID;
...
AutoBalanceStatModifiers getStatModifiers (Map* map, Creature* creature = nullptr)
{
    ...
    if (creature)
    {
        creatureABInfo = GetAutoBalanceCreatureInfo(creature);
    }
...
float getDefaultMultiplier(Map* map, AutoBalanceInflectionPointSettings inflectionPointSettings)
{
    ...
    AutoBalanceMapInfo *mapABInfo=GetAutoBalanceMapInfo(map);
...
void AddCreatureToMapCreatureList(Creature* creature, bool addToCreatureList = true, bool forceRecalculation = false)
{
    ...
    InstanceMap* instanceMap = map->ToInstanceMap();
    AutoBalanceMapInfo *mapABInfo=GetAutoBalanceMapInfo(instanceMap);
    AutoBalanceCreatureInfo *creatureABInfo=GetAutoBalanceCreatureInfo(creature);
    ...
    if (creature->IsSummon())
    {
            ...
            creature->ToTempSummon()->GetSummoner()->ToCreature())
        {
            ...
            {
                AutoBalanceCreatureInfo *summonerABInfo=GetAutoBalanceCreatureInfo(summoner);
        ...
        // pets and totems
        else if (creature->GetCreatorGUID().IsPlayer() || creature->IsPet() || creature->IsHunterPet() || creature->IsTotem())
        {
            LOG_DEBUG("module.AutoBalance", "AutoBalance::AddCreatureToMapCreatureList: Creature {} ({}) (summon) | is a {}. Original level set to ({}).",
                        creature->GetName(),
                        creatureABInfo->UnmodifiedLevel,
                        creature->GetCreatorGUID().IsPlayer() ? "creature created by a player" : creature->IsPet() ? "pet" : creature->IsHunterPet() ? "hunter pet" : "totem",
                        creatureABInfo->UnmodifiedLevel
            );
        }
        else
    ...
    if (isIncludedInMapStats)
    {
        ...
        if  ((creature->IsVendor() ||
                ...
                creature->HasUnitFlag(UNIT_FLAG_UNINTERACTIBLE)) &&
...
{
    // get map data
    AutoBalanceMapInfo *mapABInfo=GetAutoBalanceMapInfo(creature->GetMap());
...
class AutoBalance_WorldScript : public WorldScript
{
    ...
    void OnBeforeConfigLoad(bool /*reload*/) override
    {
        ...
        std::string configFile = "conf/mod_autobalance.conf";
        if (!sConfigMgr->LoadAdditionalFile(configFile, true, configError))
        {
            LOG_WARN("module.AutoBalance", "AutoBalance::OnBeforeConfigLoad: Unable to load '{}' ({}) - falling back to defaults.", configFile, configError);
            configError.clear();
            configFile = "conf/mod_autobalance.conf.dist";
            if (!sConfigMgr->LoadAdditionalFile(configFile, false, configError))
                LOG_ERROR("module.AutoBalance", "AutoBalance::OnBeforeConfigLoad: Failed to load fallback '{}' ({}) - using hardcoded defaults.", configFile, configError);
        }
    ...
    public:
        ...
        void OnGiveXP(Player* player, uint32& amount, Unit* victim, uint8 /*xpSource*/) override
        {
            ...
            if (victim && RewardScalingXP && mapABInfo->enabled)
            {
                ...
                AutoBalanceCreatureInfo *creatureABInfo=GetAutoBalanceCreatureInfo(victim);
        ...
        void OnBeforeLootMoney(Player* player, Loot* loot) override
        {
            ...
            if (mapABInfo->enabled && RewardScalingMoney)
            {
                ...
                if (sourceGuid.IsCreature())
                {
                    ...
                    AutoBalanceCreatureInfo *creatureABInfo=GetAutoBalanceCreatureInfo(sourceCreature);
...
class AutoBalance_UnitScript : public UnitScript
{
    ...
    private:
        ...
        int32 _Modify_Damage_Healing(Unit* target, Unit* source, int32 amount, SpellInfo const* spellInfo = nullptr)
        {
            ...
            // get the maps' info
            AutoBalanceMapInfo *sourceMapABInfo = GetAutoBalanceMapInfo(source->GetMap());
            AutoBalanceMapInfo *targetMapABInfo = GetAutoBalanceMapInfo(target->GetMap());
            ...
            else
            {
                ...
                if (_isAuraWithEffectType(spellInfo, SPELL_AURA_PERIODIC_DAMAGE_PERCENT))
                {
                    damageMultiplier = GetAutoBalanceCreatureInfo(source)->DamageMultiplier;
                    if (_debug_damage_and_healing)
                    {
                        LOG_DEBUG("module.AutoBalance_DamageHealingCC", "AutoBalance_UnitScript::_Modify_Damage_Healing: Spell damage based on percent of max health. Ignore level scaling.");
                        LOG_DEBUG("module.AutoBalance_DamageHealingCC",
                ...
                {
                    damageMultiplier = GetAutoBalanceCreatureInfo(source)->ScaledDamageMultiplier;
                    if (_debug_damage_and_healing)
                    {
                        LOG_DEBUG("module.AutoBalance_DamageHealingCC",
        ...
        uint32 _Modifier_CCDuration(Unit* target, Unit* caster, Aura* aura)
        {
            ...
            float ccDurationMultiplier = GetAutoBalanceCreatureInfo(caster)->CCDurationMultiplier;
...
public:
    ...
    bool ResetCreatureIfNeeded(Creature* creature)
    {
        ...
        if (creatureABInfo->mapConfigTime < mapABInfo->mapConfigTime)
        {
            ...
            ResetAutoBalanceCreatureInfo(creature);
            AutoBalanceCreatureInfo *creatureABInfo = GetAutoBalanceCreatureInfo(creature);
...
private:
    bool _isSummonCloneOfSummoner(Creature* summon)
    {
        ...
        AutoBalanceCreatureInfo* summonABInfo = GetAutoBalanceCreatureInfo(summon);
...
public:
    ...
    static bool HandleABMapStatsCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        AutoBalanceMapInfo *mapABInfo=GetAutoBalanceMapInfo(player->GetMap());
    ...
    static bool HandleABCreatureStatsCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        AutoBalanceCreatureInfo *targetABInfo=GetAutoBalanceCreatureInfo(target);
