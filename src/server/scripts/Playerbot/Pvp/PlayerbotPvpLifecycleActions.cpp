...
bool TryBuildBattlegroundSegmentDestination(Player* player, Position const& safeDestination, Position& segmentDestination, PathType* resolvedPathType = nullptr)
{
    ...
    auto const tryResolveDestination = [&](Position const& requestedDestination, Position& resolvedDestination, PathType* resolvedType) -> bool
    {
    ...
    for (float probeDistance : probeDistances)
    {
        ...
        if (tryResolveDestination(probeDestination, segmentDestination, resolvedPathType))
            return true;
    }
