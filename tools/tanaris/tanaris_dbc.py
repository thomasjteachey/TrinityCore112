...
ROWS = {
    ...
    "PvpDifficulty.dbc": [
        ...
        # 80-80 ON PURPOSE: the server caps at 60, so an 80-only bracket is
        # how Tanaris is currently DISABLED without deleting anything. Widen
        # to 1-80 to re-enable (2026-08-06).
        [90000 + MAP_ID, MAP_ID, 0, 80, 80, 0],
    ],
    "WorldSafeLocs.dbc": [
    ...
    "WorldMapArea.dbc": [
        # The client only positions the player arrow when the displayed area's
        # MapID equals the map the player is actually standing on; stock
        # Tanaris (row 161) says MapID 1.  The battleground also ships a baked,
        # fully revealed map under Interface\WorldMap\TanarisBG, so AreaName
        # must select that directory rather than the stock Tanaris artwork.
        #
